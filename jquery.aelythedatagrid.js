...
(function ($) {
	...
	_buildTable = function(_settings) {
		...
		// data source as html table
		if (_settings.datasource=="html_table")
		{
			$('.ad-datagrid', _settings._target).append('<table class="ad-source" style="display:none"><table>');
		}
    ...
    */
	...
	/** Use a HTML Table as Data Source **/
	_buildHTMLDatasourceRows = function(_settings, rowHead, rowData ) {
		var row = '',
			rows = '';
		$.each(rowData, function(index, val) {
			row = '';
			$.each(this, function(index1, val1) {
				if (rowHead[index1]!=undefined)
				{
					var colName = rowHead[index1];
					row += '<td col-name="' + colName + '">' + val1 + '</td>';
				}
			})
			rows += '<tr>' + row + '</tr>';
		});
		// save source
		$('table.ad-source', _settings._target).html( rows );
	}
	...
	_refreshRows = function(_settings) {
		...
		// search source html table
		if (_settings.datasource=="html_table")
		{
			var expression = '.ad-source tr';

			// filter
			if ($('.ad-filter', _settings._target).val().length > 0) {
				expression += '.match';
			}
			...
			if (_settings._num_rows>=start && _settings._num_rows<(_settings.ipp * _settings._currentPage))
			{
				...
				$.each(_settings._rawData.head, function(index1, val1){
					...
					if (_colNames[val1])
					{
						if (_settings.datasource=="html_table")
						{
							var colText = $('td[col-name="'+val1+'"]', val).text();
							table_row += '<td col-name="' + val1 + '">' + colText + '</td>';
						}
						else
						{
	...
	_searchAction = function(_settings, string) {
		//console.log(_settings._target);  
		if (_settings.datasource=="html_table")
		{
			$('table.ad-source tr', _settings._target).removeClass('match');
			if (string.length >= 0)
			{
				$.each($('table.ad-source td' , _settings._target), function (index, val) {
					if ($(this).text().toLowerCase().search(string.toLowerCase()) >= 0)
					{
		...
		// reload data with ajax for filter
		else
		{
            // Get filter value
			var filter = string;
	...
	_loadData = function(_settings) {
        if(localStorage.getItem("sha1Data"))
        {
            _settings.data.sha1 = localStorage.getItem("sha1Data");
        }
		...
		$.ajax({
			...
			success: function(responseData) {
				_dbg('data loaded OK');
				//_dbg(_settings._rawData);
				if (_settings.datasource=="html_table")
				{
					_buildHTMLDatasourceRows(_settings, _settings._rawData.head, _settings._rawData.data );
				}
                // save to lacal storage
                
                if(typeof(Storage)!=="undefined" && responseData !== false)
                {
                    console.log("diferit");
                    // Save object to lacal storage
                    localStorage.setItem('jsonData', JSON.stringify(responseData));
                    localStorage.setItem('sha1Data', responseData.sha1);
                }
                
                _settings._rawData = JSON.parse(localStorage.getItem('jsonData'));    
    ...
    */
	...
	$.fn.ADatagrid.defaults = {
		...
		_currentPage: 1,
		_pages: null,
		datasource: "json", // options: 'json' (default), 'html_table'
		url : "",
		data: {
